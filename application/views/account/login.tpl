	...
	<div class="container">
		...
		<div class="center-row" id="send_mail">
	      ...
	      <ul class="nav navbar-nav nav-body">
	        <li>
	            {form class='form-horizontal' attr=['novalidate'=>''] action="{site_url url='account/login'}" method="POST"}
					<div class="panel panel-default">
					  <div class="panel-heading">{lang}Login{/lang}</div>
					  <div class="panel-body">
					
					    {field_group  field='username' layout=['element' => 9]}{/field_group}
					    {field_group  field='password' layout=['element' => 9]}{/field_group}
                        {input field='id' type='hidden'}
					    <input class="btn pinet-btn-blue login" type="submit" value="{lang}Login{/lang}">
			   		   <a href="{$register_url}" class="btn pinet-btn-blue register">{lang}Register{/lang}</a>
					  </div>
					</div>
				{/form}
	        </li>
