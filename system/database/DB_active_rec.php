...
class CI_DB_active_record extends CI_DB_driver {
    ...
    public function where_and_and($values){
        return $this->_where_and_or($values, ' AND ', ' AND ');
    }

    public function where_and_or($values){
        return $this->_where_and_or($values, ' AND ', ' OR ');
    }

    public function where_or_and($values){
        return $this->_where_and_or($values, ' OR ', ' AND ');
    }

    protected function _where_and_or($values, $type = ' AND ', $prefix= ''){
        if(is_array($values)){
            $where_and_or = NULL;
            foreach($values as $key => $value){
                $sub_query = NULL;
                $sub_query = $this->_where_and_or_build($key, $value);
                if(!is_null($sub_query)){
                    if($where_and_or){
                        $where_and_or .= $type . $sub_query;
                    }else{
                        $where_and_or .= $sub_query;
                    }
                }
            }
            if($where_and_or){
                $prefix = (count($this->ar_where) == 0 AND count($this->ar_cache_where) == 0) ? '' : $prefix;
                $where_and_or = $prefix . '(' . $where_and_or . ')';
                $this->ar_where[] = $where_and_or;

                if ($this->ar_caching === TRUE)
                {
                    $this->ar_cache_where[] = $where_and_or;
    ...
    protected function _where_and_or_build($key, $value = '', $type = ' AND ', $escape = TRUE){
        if ( ! is_array($key))
        {
            $key = array($key => $value);
        }
        ...
        foreach ($key as $k => $v)
        {
            ...
            if($this_or){
                if($where_and_or){
                    $where_and_or .= $type . $this_or;
                }else{
                    $where_and_or = $this_or;
                }
            }
        }
        if(strpos($where_and_or, $type) !== FALSE){
            return '(' . $where_and_or . ')';
        }
        return $where_and_or;
    }
